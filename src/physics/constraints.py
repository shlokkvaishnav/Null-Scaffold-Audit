...
def is_physically_valid(prediction_array):

    #Negative CO2 concentrations are not possible
    if np.any(prediction_array<0):
        return False
    
    # Upper limit for CO2 concentration in ppm
...
#Calculates Henry's Law constant for CO2 in seawater based on temperature and salinity
def calculate_henrys_law_constant(T_kelvin, Salinity):
    ...
    ln_Ko = (A1 + A2/T_100 + A3*np.log(T_100) + 
