...
This document is the policy. [`AGENT_PIPELINE.md`](AGENT_PIPELINE.md) is how one Claude Code session, rotating through three roles (researcher, implementer, reviewer) one tick at a time, carries it out — coordinating through GitHub issues/PRs/labels, never through in-session memory.
