...
class SymbolicLibrary(nn.Module):
    """
    Transforms raw input features into a library of physically meaningful basis functions.
...
class L0STE(torch.autograd.Function):
    ...
    on continuous networks during gradient descents.
    """
    @staticmethod
...
class SymbolicGate(nn.Module):
    """
    Structured composition of oceanographic primitives routing observational covariates
    to specific domain experts.
    """
    ...
    def sparsity_loss(self) -> torch.Tensor:
        ...
        (counting active physical covariate gates)
        """
        sparse_alpha = L0STE.apply(self.alpha, self.l0_threshold)
