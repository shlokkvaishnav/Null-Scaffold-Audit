...
class JointELBO(nn.Module):
    ...
    def __init__(self, num_regimes: int, transition_prior: float = 0.9):
        """
        Args:
            num_regimes: Number of discrete regimes
            transition_prior: Baseline persistence probability across the sequence (Markov diagonal)
        """
        super().__init__()
    ...
    def forward(
        ...
        Args:
            ...
            gate_probs: (T, Num_Regimes) Soft categorization q(z) dynamically issued from the gate
            regime_priors: Unused parameter kept for compatibility with the trainer
