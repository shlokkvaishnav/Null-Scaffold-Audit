    ...
    def _build_model(self):
        if self.backend == "gplearn":
            ...
            return GPRegressor(
                ...
                # gplearn's own default function_set is just (add, sub, mul,
                # div) -- too narrow to represent equations needing sqrt/sin/
                # cos/log (e.g. any equation with a radical or trig term is
                # structurally unreachable without these). gplearn recognizes
                # these as built-in protected string identifiers.
                function_set=tuple(
                    self.config.get(
