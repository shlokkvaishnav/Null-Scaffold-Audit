...
def plot_hypothesis_growth(metrics, output_dir):
    ...
    plt.plot(baseline["iterations"], baseline["num_hypotheses"], 
             marker='o', linewidth=2, label='Baseline (with pruning)', color='#2E86AB')
    
    # No memory (no pruning)
    no_mem = metrics["no_memory"]
    plt.plot(no_mem["iterations"], no_mem["num_hypotheses"], 
             marker='s', linewidth=2, label='No Memory (unbounded)', color='#A23B72')
...
def plot_entropy_comparison(metrics, output_dir):
    ...
    plt.axhline(max_entropy, color='red', linestyle='--', linewidth=2, 
    ...
    plt.title('Belief Concentration Requires Verification & Updates', fontsize=14, fontweight='bold')
...
def create_equation_table(metrics, output_dir):
    ...
    plt.title('Symbolic Interpretability Across Ablations', 
