...
from climate_discovery.config import TRAIN_NC, TEST_NC, FEATURES_ALL, FEATURES_PHYS, FEATURES_TIME, TARGET, LAT_BANDS
...
def main():
    ...
    if not phys_cols:
        phys_cols = [c for c in ["sst", "sss", "sin_month", "cos_month", "year_feature"] if c in feats]
    ...
    for name, use_bio, Xtr, Xte in [
        ...
        rows.append({"ablation": name, "bio": use_bio, "r2": r2, "rmse": rmse, "frac_in_range": p["frac_in_range"]})
        if name == "rf_bio":
            pred_rf_bio = pred
    ...
    for band, v in ood_slices(lat_te, y_te, pred_rf_bio, LAT_BANDS).items():
        rows.append({"ablation": f"ood_{band}", "bio": True, "r2": v["r2"], "rmse": v["rmse"]})
