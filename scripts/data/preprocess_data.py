"""Preprocess raw data: align, merge, split train/test, normalize.
...
def add_cyclic_time(ds):
    ...
    # normalized linear trend (used only by experts, not gating)
...
def preprocess():
    ...
    # Fix longitude range
    # -----------------------
    lon = ds_socat.coords["lon"]
    if float(lon.max()) > 180:
        ds_socat = ds_socat.assign_coords(
            lon=(lon + 180) % 360 - 180
        ).sortby("lon")
    ...
    chl = np.log10(ds_chl[chl_var].interp_like(ds_socat) + 1e-6)
    chl.name = "log_chl"
    ...
    # ======================================================
    # 🔑 Spatial + weak temporal features for GATING
    # ======================================================

    # Normalized latitude
    ds_merged["lat_norm"] = ds_merged["lat"] / 90.0

    # Cyclic longitude encoding (dateline-safe)
    lon_rad = np.deg2rad(ds_merged["lon"])
    ds_merged["sin_lon"] = np.sin(lon_rad)
    ds_merged["cos_lon"] = np.cos(lon_rad)

    # Weak seasonal modulation (NO phase info)
    ds_merged["season_strength"] = np.abs(ds_merged["sin_month"])
    ...
    logger.info(
        "Train steps: %d | Test steps: %d",
        len(ds_train.time),
        len(ds_test.time),
    )

    # -----------------------
    # Standardize physical variables ONLY
    # -----------------------
    scalers = xr.Dataset()
    for var in ["sst", "sss", "log_chl"]:
        m = ds_train[var].mean(dim=("time", "lat", "lon"))
        s = ds_train[var].std(dim=("time", "lat", "lon"))
    ...
    ds_train.to_netcdf(
        TRAIN_OUTPUT_PATH, encoding={v: comp for v in ds_train.data_vars}
    )
    ds_test.to_netcdf(
        TEST_OUTPUT_PATH, encoding={v: comp for v in ds_test.data_vars}
    )
